 ...
 */
function tempoAus(wert: unknown): Fortschritt["tempo"] {
  const tempo: Fortschritt["tempo"] = {};
  if (typeof wert !== "object" || wert === null) return tempo;

  const geprueft: [string, { sekunden: number; anzahl: number }][] = [];
 ...
 */
export function zumSpeichern(fortschritt: Fortschritt): Record<string, unknown> {
