 ...
 * `breite` × `hoehe` gezogen. Formen, die rund bleiben müssen (Wecker, Ente,
 * Katze), bekommen deshalb ein fast quadratisches Feld; Möbel dürfen breit
 * sein. Strich statt Fläche wird vermieden: Beim Ziehen würden aus gleich
 * dicken Strichen ungleich dicke.
 */
export interface Sticker {
  /** Stabile Nummer – sie steht im Spielstand und darf sich nie ändern. */
  nummer: number;
  /** Für die Auswahlkarte und den Screenreader. */
  name: string;
  /** Mitte des Platzes im Bild. */
  x: number;
  y: number;
  breite: number;
  hoehe: number;
...
function kasten(farbe: string): string {
  return (
    `<rect x="2" y="2" width="96" height="96" rx="8" class="bild-dunkel"/>` +
...
export const STICKER: readonly Sticker[] = [
...
const NACH_NUMMER = new Map<number, Sticker>(STICKER.map((s) => [s.nummer, s]));

/** Gibt es diesen Sticker? */
export function istStickerNummer(wert: unknown): wert is number {
  return typeof wert === "number" && NACH_NUMMER.has(wert);
}

export function sticker(nummer: number): Sticker {
  const gefunden = NACH_NUMMER.get(nummer);
  if (!gefunden) throw new Error(`Unbekannter Sticker: ${nummer}`);
  return gefunden;
}
 ...
 */
export function sammelbild(
  gesammelt: readonly number[],
  neu: number | null = null,
  zeigeLuecken = true
): string {
  const geklebte = new Set<number>(gesammelt);
  let teile = haus();
  for (const eintrag of STICKER) {
    if (geklebte.has(eintrag.nummer)) teile += geklebt(eintrag, eintrag.nummer === neu);
    else if (zeigeLuecken) teile += luecke(eintrag);
  }
  return huelle(teile);
}

/** Ein Sticker allein, für die Auswahlkarte. */
export function stickerBild(nummer: number): string {
  const eintrag = sticker(nummer);
  return (
    `<svg viewBox="0 0 100 100" class="illu" xmlns="http://www.w3.org/2000/svg" ` +
