...
/**
 * Alles, was gezeichnet werden kann – inklusive der Ellipse. Die steht
 * bewusst NICHT in `ALLE_FORMEN`: „Ellipse“ ist kein Wort für die 2. Klasse
 * und stünde sonst sofort in „Welche Form ist das?“. Gebraucht wird sie nur
 * als Fast-Treffer unter den Kreisen.
 */
export type Zeichenform = FormName | "Ellipse";
...
/** Größe und Lage einer Figur – für „Welche Form passt nicht?“. */
export interface Formlage {
  /** Anteil der vollen Zeichnung, z. B. 0.7. */
  groesse?: number;
  /**
   * Drehung in Grad. Höchstens ±20 verwenden: Ein um 45° gedrehtes Quadrat
   * ist von einer Raute nicht mehr zu unterscheiden, und dann hätte die
   * Aufgabe zwei richtige Antworten.
   */
  drehung?: number;
}
 ...
 */
export function formVariante(name: Zeichenform, lage: Formlage = {}): string {
  ...
  return huelle(
    200,
    200,
    `<g transform="translate(100 100) ${gedreht} translate(-100 -100)">${formInhalt(name)}</g>`
  );
}
 ...
 */
function formInhalt(name: Zeichenform): string {
  switch (name) {
    case "Ellipse":
      return (
        `<ellipse cx="100" cy="100" rx="88" ry="54" class="fig-flaeche-bunt"/>` +
 ...
 */
export function zahlenmauer(reihen: readonly (readonly (number | null)[])[]): string {
...
export interface RadFeld {
  /** Zahl im äußeren Ring. */
  aussen: number;
  /** Ergänzung im inneren Ring; `null` markiert das gesuchte Feld. */
  innen: number | null;
}

/** Rechenrad: außen + innen ergibt immer die Zahl in der Mitte. */
export function rechenrad(mitte: number, felder: readonly RadFeld[]): string {
  const m = 130;
  const rMitte = 40;
  const rInnen = 76;
  const rAussen = 116;
  ...
  felder.forEach((feld, i) => {
    ...
    teile += beschriftung(
      ...
      (rMitte + rInnen) / 2,
      (w1 + w2) / 2,
      gesucht ? "?" : String(feld.innen),
      gesucht ? "fig-text-marke" : "fig-text"
    );
  });
...
function beschriftung(m: number, radius: number, winkel: number, text: string, klasse: string): string {
...
function sektor(cx: number, cy: number, rI: number, rA: number, w1: number, w2: number): string {
  ...
  return (
    `M${r(ax1)},${r(ay1)} A${rA},${rA} 0 ${gross} 1 ${r(ax2)},${r(ay2)} ` +
 ...
 */
export function rechendreieck(
  innen: readonly (number | null)[],
  aussen: readonly (number | null)[],
  gesucht: { bereich: "innen" | "aussen"; feld: number }
): string {
  const A = [165, 40] as const; // Spitze
  const B = [50, 235] as const; // unten links
  const C = [280, 235] as const; // unten rechts
  const mitte = [165, 170] as const;
  ...
  const dreieck =
    `<path d="M${A[0]},${A[1]} L${C[0]},${C[1]} L${B[0]},${B[1]} Z" class="fig-flaeche"/>` +
   ...
   */
  const inhalt = (wert: number | null, istGesucht: boolean): string =>
    istGesucht ? "?" : wert === null ? "" : String(wert);
  ...
  const kreise = innen
    ...
    .map((wert, i) => {
      ...
      return (
        `<circle cx="${x}" cy="${y}" r="25" class="${istGesucht ? "fig-stein-luecke" : "fig-rad-innen"}"/>` +
  ...
  const kaesten = aussen
    ...
    .map((wert, i) => {
      ...
      return (
        `<rect x="${bx}" y="${by}" width="54" height="38" rx="6" class="${istGesucht ? "fig-stein-luecke" : "fig-fahne"}"/>` +
 ...
 */
function gerahmt(breite: number, hoehe: number, inhalt: string): string {
  const rand = 2;
 ...
 */
export function hunderterfeldStueck(felder: readonly Ausschnittfeld[]): string {
 ...
 */
export function rechenstrich(
  von: number,
  bis: number,
  markiert: number,
  mitteBeschriftet: boolean
): string {
  const breite = 520;
  const links = 40;
  ...
  const y = 62;
  const mitte = (von + bis) / 2;
  const stelle = (wert: number): number => links + ((rechts - links) * (wert - von)) / (bis - von);
