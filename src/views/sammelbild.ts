...
export const zeige: RouteHandler = (ziel) => {
  ...
  const bild = el(
    "section",
    { class: "karte" },
    el("h1", { class: "seiten-titel", text: "Dein Sammelbild" }),
    svgBild(
      sammelbild([...geklebt]),
      `Ein Haus im Wald mit ${geklebt.size} von ${STICKER_ANZAHL} eingeklebten Stickern`
    ),
    el(
      "p",
  ...
  const liste = el(
    "section",
    { class: "karte" },
    el("h2", { class: "abschnitt-titel", text: "Alle Sticker" }),
    el(
      "ul",
      { class: "stickerliste" },
      ...STICKER.map((eintrag) =>
        el(
          "li",
          { class: `stickerliste-punkt${geklebt.has(eintrag.nummer) ? "" : " stickerliste-offen"}` },
          el("span", { class: "stickerliste-name", text: eintrag.name }),
