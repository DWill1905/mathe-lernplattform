...
interface Sitzung {
  ...
  puzzle: Puzzle | null;
  /** Wie oft in DIESER Runde schon gejubelt wurde – bestimmt die nächste Art. */
  jubelZaehler: number;
  /** Startpunkt der Reihenfolge, damit nicht jede Runde gleich beginnt. */
  jubelStart: number;
  ...
  tippOffen: boolean;
  /**
   * Was gerade gefragt wird. Standard ist IMMER die eigentliche Aufgabe; in
   * die Hilfsaufgabe wechselt nur, wer den Bonus selbst antippt.
   */
...
export const zeige: RouteHandler = (ziel, parameter) => {
  ...
  const wunsch =
    ersteRoute === "rechenmeister" ? "meister" : ersteRoute === "puzzle" ? "puzzle" : (parameter[1] ?? "mix");
...
function baueRunde(wunsch: string): Sitzung | null {
  ...
  if (wunsch === "puzzle") {
    ...
    return neueSitzung(
      null,
      2,
      gemischteRunde(rng, stufen, PUZZLE_TEILE, undefined, wiederholen, zuletzt),
      false,
      waehleMotiv(zufallsSeed())
    );
  }
 ...
 */
function aufgabeZuruecksetzen(sitzung: Sitzung): void {
  sitzung.phase = "haupt";
...
function bonusMoeglich(sitzung: Sitzung): boolean {
...
function zeichne(ziel: HTMLElement, sitzung: Sitzung): void {
  ...
  if (sitzung.puzzle) {
    ...
    if (zaehler) reihe.appendChild(zaehler);
  ...
  if (bonusMoeglich(sitzung) && aufgabe.vorstufe) {
    karte.appendChild(
      el(
        ...
        icon("herz", "herz-klein"),
        "Hilfsaufgabe selbst rechnen"
      )
    );
  }
...
function puzzlestreifen(sitzung: Sitzung, puzzle: Puzzle): HTMLElement {
  ...
  const rahmen = el(
    "div",
    { class: `puzzle-rahmen${sitzung.beantwortet ? " frisch" : ""}` },
...
function zeichneErgebnis(ziel: HTMLElement, sitzung: Sitzung): void {
  ...
  if (sitzung.puzzle) {
    ...
    karte.append(
      ...
      el(
        "div",
        { class: "puzzle-rahmen puzzle-rahmen-gross" },
        svgBild(puzzleBild(sitzung.puzzle, stand), `Fertiges Puzzle: ${sitzung.puzzle.name}`)
      ),
      el("p", {
