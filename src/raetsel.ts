...
export interface Raetsel {
  wort: string;
  satz: string;
  /** Legende, alphabetisch sortiert wie im Heft. */
  code: CodeEintrag[];
  /** Die Legende als fertiges Bild. */
  codeBild: string;
  /** Eine Aufgabe je Buchstabe, in der Reihenfolge des Worts. */
  aufgaben: Aufgabe[];
}
 ...
 * bekommt.
 */
export function aufgabeFuerZahl(rng: Rng, ziel: number, verbraucht: Set<string>): Aufgabe {
  for (let versuch = 0; versuch < 20; versuch++) {
    ...
    if (verbraucht.has(rechnung) && versuch < 19) continue;
...
export function baueRaetsel(rng: Rng): Raetsel {
  ...
  const zahlen = rng
    ...
    .sort((a, b) => a - b);
  const code: CodeEintrag[] = buchstaben.map((buchstabe, i) => ({ buchstabe, zahl: zahlen[i]! }));
