 ...
 */
const THEMENBILDER: Record<string, string> = {
...
export interface Puzzle {
  /** Name des Motivs, für die Rückmeldung am Ende. */
  name: string;
  /** Das vollständige Bild als SVG-Zeichenkette. */
  svg: string;
  spalten: number;
  zeilen: number;
}
...
/** Zustand eines einzelnen Puzzleteils. */
export type Teilstand = "zu" | "auf" | "grau";
 ...
 */
export function puzzleStaende(
  index: number,
  aktuelleFertig: boolean,
  ergebnisse: readonly boolean[],
  teile: number = PUZZLE_TEILE
): Teilstand[] {
...
/** Wählt ein Motiv aus. */
export function waehleMotiv(zahl: number): Puzzle {
...
export function motivNamen(): string[] {
 ...
 */
export function puzzleBild(puzzle: Puzzle, stand: readonly Teilstand[]): string {
