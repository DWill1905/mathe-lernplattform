 ...
 * genau falsch.
 */
export function neuerFamilienCode(zufall: (anzahl: number) => Uint8Array = kryptoZufall): string {
  const werte = zufall(CODE_LAENGE);
  let code = "";
  for (let i = 0; i < CODE_LAENGE; i++) code += ZEICHEN[werte[i]! % ZEICHEN.length];
  return code;
}

function kryptoZufall(anzahl: number): Uint8Array {
...
export function normalisiereCode(eingabe: string): string | null {
  ...
  for (const zeichen of code) if (!ZEICHEN.includes(zeichen)) return null;
  return code;
}

export function familienCode(): string | null {
  try {
...
export function setzeFamilienCode(code: string | null): void {
  try {
...
export function zuletztAbgeglichen(): string | null {
  try {
 ...
 */
export function verschmelze(a: Fortschritt, b: Fortschritt): Fortschritt {
...
function besteZeit(x: number, y: number): number {
  if (x === 0) return y;
  if (y === 0) return x;
  return Math.min(x, y);
}
...
function verschmelzeVerlauf(
  a: Fortschritt["verlauf"],
  b: Fortschritt["verlauf"]
): Fortschritt["verlauf"] {
...
export async function sendeStand(
  code: string,
  stand: Fortschritt,
 ...
 */
export async function gleicheAb(hole: FetchLike = fetch): Promise<AbgleichErgebnis> {
  const code = familienCode();
  if (!eingerichtet() || !code) return { art: "aus" };
 ...
 */
export async function abgleichMit(code: string, hole: FetchLike = fetch): Promise<AbgleichErgebnis> {
  try {
