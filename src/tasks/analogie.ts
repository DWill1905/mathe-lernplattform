...
export function analogie(rng: Rng, stufe: Stufe): Aufgabe {
  if (stufe === 1) {
    ...
    if (wahl === 1) return nachbarPlus(rng);
  ...
  if (stufe === 2) {
    ...
    if (wahl === 3) return nachbarMinus(rng);
  ...
  if (wahl === 4) return mitUebergangDavor(rng);
 ...
 * Analogieaufgaben.
 */
function nachbarPlus(rng: Rng): Aufgabe {
  const a = rng.int(2, 9);
  const doppelt = a + a;
...
function nachbarMinus(rng: Rng): Aufgabe {
 ...
 */
function mitUebergangDavor(rng: Rng): Aufgabe {
  const plus = rng.chance(0.5);
