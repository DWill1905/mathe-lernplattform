...
type Namen = readonly [string, string];

export function sachaufgaben(rng: Rng, stufe: Stufe): Aufgabe {
  if (stufe === 1) return rng.pick([dazubekommen, weggeben, mehrAls])(rng, 20);
...
const DAZU: readonly ((a: number, b: number, n: Namen) => string)[] = [
...
const WEG: readonly ((a: number, b: number, n: Namen) => string)[] = [
...
const MEHR: readonly ((a: number, b: number, n: Namen) => string)[] = [
...
const VERTEILEN: readonly ((gesamt: number, gruppen: number, n: Namen) => string)[] = [
...
const PACKUNGEN: readonly ((pro: number, anzahl: number) => string)[] = [
...
const ZWEI_SCHRITTE: readonly ((start: number, dazu: number, weg: number, n: Namen) => string)[] = [
...
const EINKAUF: readonly ((stueck: number, preis: number, gezahlt: number, n: Namen) => string)[] = [
...
interface Vergleich {
  /** Nennt beide Mengen. */
  einleitung: (a: number, b: number, n: Namen) => string;
...
const VERGLEICH: readonly Vergleich[] = [
  {
...
function zusammenUndDifferenz(rng: Rng, _max: number): Aufgabe {
  ...
  return {
    typ: "sach/vergleich",
    frage: `${geschichte.einleitung(a, b, namen)} ${
      fragtDifferenz ? geschichte.mehr(namen) : geschichte.zusammen()
    }`,
    antwortfeld: zahlfeld(),
