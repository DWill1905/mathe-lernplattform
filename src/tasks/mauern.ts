...
export function mauern(rng: Rng, stufe: Stufe): Aufgabe {
  if (stufe === 1) return rng.chance(0.6) ? kleineMauer(rng, false) : rad(rng, stufe);
  if (stufe === 2) return rng.chance(0.6) ? kleineMauer(rng, true) : rad(rng, stufe);
...
function grosseMauer(rng: Rng): Aufgabe {
  ...
  return {
    ...
    erklaerung:
      `Von unten gerechnet: ${a} + ${b} = ${a + b} und ${b} + ${c} = ${b + c}, ` +
