...
function zehnerEiner(rng: Rng): Aufgabe {
...
function zerlegen(rng: Rng): Aufgabe {
...
function vergleich(rng: Rng): Aufgabe {
...
function nachbarzehner(rng: Rng): Aufgabe {
...
function folge(rng: Rng): Aufgabe {
...
function strahl(rng: Rng): Aufgabe {
 ...
 */
function pfeilkette(rng: Rng): Aufgabe {
  for (let versuch = 0; versuch < 40; versuch++) {
    ...
    const werte = [start];
    for (let i = 0; i < 5; i++) werte.push(werte[i]! + (i % 2 === 0 ? a : b));
    if (werte.some((wert) => wert < 0 || wert > 100)) continue;
    ...
    return {
      ...
      bild: {
        svg: pfeilfolge(
          werte.map((wert, i) => (i === luecke ? null : wert)),
          schritte
        ),
...
function runden(rng: Rng): Aufgabe {
  let zahl = rng.int(11, 94);
  while (zahl % 10 === 0 || zahl % 10 === 5) zahl = rng.int(11, 94);
...
function groesste(rng: Rng): Aufgabe {
  const zahlen = new Set<number>();
  while (zahlen.size < 4) zahlen.add(rng.int(21, 99));
  const liste = [...zahlen];
...
function mitte(rng: Rng): Aufgabe {
...
function folgeSchwer(rng: Rng): Aufgabe {
...
function zwischenZehnern(rng: Rng): Aufgabe {
  ...
  for (let versuch = 0; versuch < 40 && ablenker.length < 6; versuch++) {
    const zahl = rng.int(1, 100);
    if (zahl > unten && zahl < unten + 10) continue;
    if (!ablenker.includes(String(zahl))) ablenker.push(String(zahl));
  }
  return {
...
function ordnen(rng: Rng): Aufgabe {
  ...
  for (let versuch = 0; versuch < 60 && ablenker.length < 5; versuch++) {
    ...
    if (reihe !== loesung && !ablenker.includes(reihe)) ablenker.push(reihe);
  }
  return {
...
export function plusminus(rng: Rng, stufe: Stufe): Aufgabe {
  if (stufe === 1) {
    ...
    if (wahl === 1) return mehrereSummanden(rng, 20);
    if (wahl === 2) return ergaenzeZumZehner(rng, 1);
    if (wahl === 3) return volleZehner(rng);
    return bis20(rng);
  }
  if (stufe === 2) {
    if (rng.chance(0.25)) return ergaenzeZumZehner(rng, 2);
  ...
  if (rng.chance(0.25)) return tabelle(rng, rng.chance(0.4));
  if (rng.chance(0.2)) return mehrereSummanden(rng, 100);
  return rng.pick([mitUebergang, mitUebergang, platzhalter])(rng);
}

function bis20(rng: Rng): Aufgabe {
...
function volleZehner(rng: Rng): Aufgabe {
  const plus = rng.chance(0.5);
  if (plus) {
    const a = rng.int(1, 8) * 10;
    const b = rng.int(1, (100 - a) / 10) * 10;
    return rechenaufgabe(
      "plusminus/zehner",
      a,
      b,
      "+",
      a + b,
      `Rechne in Zehnern: ${a / 10} + ${b / 10} = ${(a + b) / 10} Zehner.`
    );
  }
...
function zehnerPlus(rng: Rng): Aufgabe {
  const a = rng.int(11, 79);
  const b = rng.int(1, Math.floor((99 - a) / 10)) * 10;
  return rechenaufgabe(
    "plusminus/zehner-dazu",
    a,
    b,
    "+",
    a + b,
    "Beim Zehner-Dazuzählen bleibt die Einerstelle gleich."
  );
}

function ohneUebergang(rng: Rng): Aufgabe {
  const plus = rng.chance(0.5);
  if (plus) {
    ...
    return rechenaufgabe(
      "plusminus/ohne-uebergang",
      a,
      b,
      "+",
      a + b,
      `Erst die Zehner: ${zehnerA * 10} + ${zehnerB * 10} = ${(zehnerA + zehnerB) * 10}. Dann die Einer: ${einerA} + ${einerB} = ${einerA + einerB}.`
    );
  }
...
function mitUebergang(rng: Rng): Aufgabe {
  const plus = rng.chance(0.5);
  if (plus) {
    ...
    return rechenaufgabe(
      "plusminus/mit-uebergang",
      a,
      b,
      "+",
      a + b,
      `Erst zum nächsten Zehner: ${a} + ${bisZehner} = ${a + bisZehner}. Dann der Rest: ${a + bisZehner} + ${b - bisZehner} = ${a + b}.`
    );
  }
  const zehnerA = rng.int(3, 9);
  ...
  return rechenaufgabe(
    ...
    "−",
    a - b,
    `Erst bis zum Zehner: ${a} − ${einerA} = ${a - einerA}. Dann der Rest: ${a - einerA} − ${b - einerA} = ${a - b}.`
  );
}

function platzhalter(rng: Rng): Aufgabe {
 ...
 */
function mehrereSummanden(rng: Rng, max: number): Aufgabe {
...
function schrittweise(werte: readonly number[]): string {
 ...
 * mit Zehnerübergang.
 */
function ergaenzeZumZehner(rng: Rng, stufe: 1 | 2): Aufgabe {
...
export function knobeln(rng: Rng, stufe: Stufe): Aufgabe {
  if (stufe === 1) return rng.pick([verdoppeln, halbieren])(rng);
  if (stufe === 2) return rng.pick([geradeUngerade, muster, nachbarSumme])(rng);
  return rng.pick([raetselPlus, raetselMal, raetselMinus])(rng);
}

function verdoppeln(rng: Rng): Aufgabe {
...
function halbieren(rng: Rng): Aufgabe {
...
function geradeUngerade(rng: Rng): Aufgabe {
  const zahl = rng.int(11, 99);
...
function muster(rng: Rng): Aufgabe {
...
function nachbarSumme(rng: Rng): Aufgabe {
...
function raetselPlus(rng: Rng): Aufgabe {
...
function raetselMinus(rng: Rng): Aufgabe {
...
function raetselMal(rng: Rng): Aufgabe {
