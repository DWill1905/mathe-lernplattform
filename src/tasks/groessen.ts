...
function betragLegen(rng: Rng): Aufgabe {
  ...
  for (let versuch = 0; versuch < 120 && ablenker.length < 3; versuch++) {
    ...
    const gleicheKlasse = EURO_STUECKE.filter(
      (wert) => wert !== bisher && wert >= 10 === bisher >= 10
    );
    // Erst die Hälfte der Versuche streng, danach notfalls auch quer.
    const vorrat =
      gleicheKlasse.length > 0 && versuch < 60
        ? gleicheKlasse
    ...
    if (summe === ziel || gesehen.has(schluessel)) continue;
...
const EURO_STUECKE: readonly number[] = [1, 2, 5, 10, 20];

/** Greift zu den größten Scheinen und Münzen zuerst – wie beim Bezahlen. */
function euroStuecke(betrag: number): number[] {
  const teile: number[] = [];
  let rest = betrag;
...
function alsZeile(stuecke: readonly number[]): string {
...
function preisUnterschied(rng: Rng): Aufgabe {
...
function laengeVergleichen(rng: Rng): Aufgabe {
  ...
  while (cmWert === mWert * 100) cmWert = rng.int(30, 250);
