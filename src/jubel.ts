...
export const BONUS_JUBEL: JubelArt = "pferd";
 ...
 */
export const ALLE_JUBEL_ARTEN: readonly JubelArt[] = [...JUBEL_ARTEN, BONUS_JUBEL];
 ...
 */
const DAUER: Record<JubelArt, number> = {
  zauberhut: 1900,
  schwein: 2300,
  biene: 2300,
  vogel: 2300,
 ...
 */
export function quert(art: JubelArt): boolean {
...
function funke(x: number, y: number, groesse: number, klasse: string): string {
  const g = groesse;
  return (
    `<path d="M${x} ${y - g}q${g * 0.2} ${g * 0.8} ${g} ${g}q-${g * 0.8} ${g * 0.2} -${g} ${g}` +
...
function funkenreihe(punkte: readonly (readonly [number, number, number])[]): string {
  return punkte
...
function schwein(): string {
  return huelle(
    120,
    80,
    `<ellipse cx="62" cy="48" rx="34" ry="24" class="bild-rosa"/>` +
...
function biene(): string {
  return huelle(
    120,
    80,
    `<ellipse cx="62" cy="48" rx="32" ry="22" class="bild-gelb"/>` +
...
function vogel(): string {
  return huelle(
    120,
    80,
    `<ellipse cx="56" cy="46" rx="32" ry="21" class="bild-tuerkis"/>` +
 ...
 */
function beinpaar(x: number, klasse: string): string {
  const bein = (bx: number): string =>
    `<rect x="${bx}" y="46" width="8" height="26" rx="4" class="bild-fell"/>` +
...
function rakete(): string {
  return huelle(
    200,
    150,
    `<g class="jubel-rakete">` +
...
export function jubelSvg(art: JubelArt): string {
  return BAUER[art]();
}
 ...
 */
export function waehleJubel(zahl: number): JubelArt {
...
let laufend: { knoten: HTMLElement; uhr: ReturnType<typeof setTimeout> } | null = null;

/** Räumt eine laufende Jubelebene sofort weg. */
export function raeumeJubel(): void {
  if (!laufend) return;
 ...
 */
export function zeigeJubel(art: JubelArt): void {
