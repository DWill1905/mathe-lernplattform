...
export function frischeShellAuf(): void {
  const kopf = document.getElementById("kopf");
  const fuss = document.querySelector<HTMLElement>(".navigation");
  if (!kopf || !fuss) return;
  ...
  kopf.append(
    ...
    el(
      ...
      el(
        "div",
        { class: "chip chip-level", title: `${fortschritt.punkte} Punkte` },
        el("span", { class: "chip-zahl", text: level.stufe }),
        el("span", { class: "chip-text", text: level.titel })
      ),
      fortschritt.streakTage > 0 &&
        el(
          "div",
