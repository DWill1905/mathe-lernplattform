 ...
 */
export function rechnungSprechen(rechnung) {
    let text = rechnung;
    for (const [muster, ersatz] of ZEICHEN)
 ...
 */
export function aufgabeSprechen(aufgabe) {
    const teile = [saubere(aufgabe.frage)];
    if (aufgabe.rechnung)
        teile.push(rechnungSprechen(aufgabe.rechnung));
...
export function setzeVorlesen(an) {
    try {
        if (an)
...
export function vorlesenMoeglich() {
...
function deutscheStimme() {
    return waehleStimme(speechSynthesis.getVoices());
}
/**
