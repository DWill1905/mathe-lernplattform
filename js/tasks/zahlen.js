import { hunderterfeld, hunderterfeldStueck, pfeilfolge, rechenstrich, rechentabelle, zahlenstrahl, } from "../figures.js";
...
function ankerZahlen(stufe) {
    ...
    if (stufe === 1)
        return [1, 11, 21, 31, 41, 51, 61, 71, 81, 91, ...zehnerspalte];
 ...
 */
function hundertfeld(rng, stufe) {
    const anker = ankerZahlen(stufe);
    let loesung = rng.int(1, 100);
    while (anker.includes(loesung))
        loesung = rng.int(1, 100);
 ...
 */
function hundertfeldMuster(rng, stufe) {
    const anker = ankerZahlen(stufe);
    ...
    for (let versuch = 0; versuch < 60; versuch++) {
        ...
        if (anker.includes(loesung))
            continue;
        if (bunt.filter((zahl) => anker.includes(zahl)).length > 1)
            continue;
        break;
    }
    return {
...
function feldStueck(rng) {
    ...
    const stellen = bau.stellen.map(([z, s]) => [zeile + z, spalte + s]);
 ...
 * (Heft, Aufgabe 2).
 */
function strichAblesen(rng, stufe) {
    ...
    while (mitteBeschriftet && loesung === mitte)
        loesung = rng.int(von + 1, bis - 1);
 ...
 */
function nachbarzehnerPaar(rng) {
    let zahl = rng.int(11, 89);
    while (zahl % 10 === 0)
        zahl = rng.int(11, 89);
...
const SCHRITTWEITEN = [1, 2, 3, 4, 5, 10];
/** Eine Zahlenreihe mit festem Schritt, immer im Zahlenraum bis 100. */
function reiheMitSchritt(rng, schritt, aufwaerts, glieder = 4) {
...
function regelReihe(rng) {
    ...
    for (let versuch = 0; versuch < 60 && ablenker.length < 5; versuch++) {
        ...
        if (!passt(reihe) && !ablenker.includes(reihe))
            ablenker.push(reihe);
    }
    return {
 ...
 */
function ziffernfamilie(rng, anzahl = 4) {
    ...
    const kandidaten = new Set(ziffern);
    for (const zehner of ziffern) {
        for (const einer of [...ziffern, 0])
            kandidaten.add(zehner * 10 + einer);
    }
