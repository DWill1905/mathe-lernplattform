...
export function uhrzeit(rng, stufe) {
    if (stufe === 1)
        return rng.chance(0.4) ? welcheUhr(rng, [0, 30]) : uhrAblesen(rng, [0, 30]);
    if (stufe === 2)
...
export function laengen(rng, stufe) {
    if (stufe === 1)
        return rng.pick([meterInCm, cmInMeter, bisZumMeter, bisZumMeter])(rng);
    if (stufe === 2)
 ...
 */
const LANGE_DINGE = ["Das Band", "Das Seil", "Die Schnur", "Die Leiste"];
function bisZumMeter(rng) {
