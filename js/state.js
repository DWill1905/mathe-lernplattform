...
export function pruefeFortschritt(roh) {
    ...
    if (typeof rohFehler === "object" && rohFehler !== null) {
        const geprueft = Object.entries(rohFehler)
            .map(([typ, anzahl]) => [typ.slice(0, 60), ganzeZahl(anzahl, 1, MAX_PUNKTE, 0)])
 ...
 */
function schluesselListe(wert) {
    if (!Array.isArray(wert))
        return [];
    return wert
        .filter((e) => typeof e === "string" && e.length > 0 && e.length <= MAX_SCHLUESSEL)
        .slice(-MAX_LETZTE);
}
/**
 ...
 */
export function merkeGestellteAufgaben(schluessel) {
